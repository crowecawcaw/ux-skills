...
(function () {
  ...
  function renderConfirmation() {
    ...
    $("#confirm-summary").innerHTML =
      ...
      '<div class="cost-row"><span>Estimated tax</span><span>' + money(tax) + "</span></div>" +
      '<div class="cost-row total"><span>Total paid</span><span>' + money(total) + "</span></div>";
  }

  function populateShippingForm() {
    if (!shippingData.name) return;
    var form = $("#form-shipping");
    $all("input", form).forEach(function (input) {
      if (shippingData[input.name] !== undefined) input.value = shippingData[input.name];
    });
  }

  function populatePaymentForm() {
    if (!paymentData.card) return;
    var form = $("#form-payment");
    $all("input", form).forEach(function (input) {
      if (paymentData[input.name] !== undefined) input.value = paymentData[input.name];
    });
  }
  ...
  function goTo(step) {
    ...
    if (step === "shipping") populateShippingForm();
    if (step === "payment") populatePaymentForm();
  ...
  function updateIndicator(current) {
    ...
    $all("#step-indicator .step").forEach(function (el) {
      ...
      if (i < idx) {
        ...
        el.setAttribute("aria-label", "Go to " + el.querySelector(".label").textContent);
        el.onclick = function () { goTo(s); };
        el.onkeydown = function (e) { if (e.key === "Enter" || e.key === " ") goTo(s); };
      } else {
        el.style.cursor = "";
