...
(function () {
  ...
  function summarize(days) {
    ...
    // Spend efficiency varies independently: prior-period spend rate is slightly
    // higher (less efficient), so the delta diverges from pure revenue movement.
    const prevSpend = prevRevenue * SPEND_RATE * 1.031;
    const conversions = revenue / ORDER_VALUE;
    // Conversion rate also varies independently: prior-period avg order value
    // was slightly lower, so conversion count delta differs from revenue delta.
    const prevConversions = prevRevenue / (ORDER_VALUE * 0.962);
  ...
  function renderKpi(id, opts) {
    ...
    if (opts.deltaGoodWhenUp === false) {
      // F2 fix: for spend, drop the +/- sign that conflicts with color/arrow direction.
      // Use plain "Spend up/down X%" so the direction and its evaluation are unambiguous.
      const absD = Math.abs(d).toFixed(1);
      const dirWord = d > 0 ? "Spend up" : "Spend down";
      deltaEl.textContent = dirWord + " " + absD + "% vs prior period";
    ...
    // Spend bar uses a neutral utilization style so it doesn't conflict with
    // the green/amber/red performance signal from the delta arrow.
    if (opts.neutralBar) {
      barEl.style.width = Math.max(4, Math.min(100, ratio * 100)) + "%";
      barEl.className = "kpi-bar-fill fill-neutral";
    } else {
      const status = statusFor(ratio);
      barEl.style.width = Math.max(4, Math.min(100, ratio * 100)) + "%";
      barEl.className = "kpi-bar-fill fill-" + status;
      // F4 fix: add a small status badge below the bar so status is readable as text, not color alone.
      let statusBadge = card.querySelector('.kpi-status-badge');
      if (!statusBadge) {
        statusBadge = document.createElement('div');
        statusBadge.className = 'kpi-status-badge';
        barEl.parentElement.after(statusBadge);
      }
      const statusLabel = status === "good" ? "Ahead of target" : status === "warn" ? "Close to target" : "Behind target";
      statusBadge.textContent = statusLabel;
      statusBadge.className = "kpi-status-badge status-" + status;
    }
  }
  ...
  function renderHeadline(s) {
    ...
    // F1 fix: "strongest" was ambiguous — ranked by efficiency (perf ratio) but bar chart showed revenue.
    // Clarify with two dimensions: who earns most (Paid Search) and who is most efficient (Email).
    const topRevChannel = [...CHANNELS].sort((a, b) => b.share - a.share)[0];
    const channelSummary = topRevChannel.name === best.name
      ? `<strong>${best.name}</strong> leads in both revenue and efficiency`
      : `<strong>${topRevChannel.name}</strong> brings in the most; <strong>${best.name}</strong> is the most efficient`;
    el.innerHTML =
      ...
      `${channelSummary}; <strong>${worst.name}</strong> needs attention.`;
  }
