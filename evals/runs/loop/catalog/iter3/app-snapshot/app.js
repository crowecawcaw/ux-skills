...
function renderEmpty() {
  ...
  // Suggestions: a few popular searches that DO return results, excluding the current query
  const ideas = ['backpack', 'tent', 'jacket', 'boots', 'water'];
  const currentQueryLower = state.query.trim().toLowerCase();
  const working = ideas.filter((q) =>
    q !== currentQueryLower &&
    PRODUCTS.some((p) => matchesQuery(p, q))
  ).slice(0, 4);
  if (working.length > 0) {
...
function updateMobileFilterBadge() {
  ...
  const label = toggleBtn.querySelector('.mobile-filter-label');
  if (label) {
    label.textContent = n > 0 ? `Filters (${n})` : 'Filters';
  }
