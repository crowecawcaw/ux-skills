...
(function () {
  ...
  function renderTrend(s) {
    ...
    const maxV = Math.max(DAILY_TARGET, ...series) * 1.04;
    // Zoom the y-axis to start just below the lowest day so day-to-day movement
    // fills the chart.
    const minV = Math.min(...series) * 0.96;
    ...
    for (let t = 0; t <= ticks; t++) {
      const v = minV + ((maxV - minV) / ticks) * t;
      const yy = y(v);
    ...
    let areaPts = `${padL},${baseY} ` + linePts + ` ${x(series.length - 1).toFixed(1)},${baseY}`;
  ...
  function renderChannels(s) {
    ...
    const total = totals.reduce((a, t) => a + t.revenue, 0);
    const palette = ["#3b5bdb", "#7048e8", "#1098ad", "#37b24d", "#f08c00"];

    const W = 260, H = 260, cx = 130, cy = 130, rOuter = 100, rInner = 56;
    function arc(a0, a1) {
      const p = (r, a) => [cx + r * Math.cos(a), cy + r * Math.sin(a)];
      const large = a1 - a0 > Math.PI ? 1 : 0;
      const [x0, y0] = p(rOuter, a0), [x1, y1] = p(rOuter, a1);
      const [x2, y2] = p(rInner, a1), [x3, y3] = p(rInner, a0);
      return `M${x0.toFixed(1)},${y0.toFixed(1)} A${rOuter},${rOuter} 0 ${large} 1 ${x1.toFixed(1)},${y1.toFixed(1)} L${x2.toFixed(1)},${y2.toFixed(1)} A${rInner},${rInner} 0 ${large} 0 ${x3.toFixed(1)},${y3.toFixed(1)} Z`;
    }
    let ang = -Math.PI / 2;
    let slices = "";
    totals.forEach((t, i) => {
      const a1 = ang + (t.revenue / total) * Math.PI * 2;
      slices += `<path d="${arc(ang, a1)}" fill="${palette[i % palette.length]}"/>`;
      ang = a1;
    });
    const svg = `<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Revenue share by channel">${slices}</svg>`;

    let legend = '<div class="donut-legend">';
    totals.forEach((t, i) => {
      legend += `<span class="dl-item"><span class="dl-swatch" style="background:${palette[i % palette.length]}"></span>${t.name} — ${fmtMoney(t.revenue)}</span>`;
    });
    legend += "</div>";

    list.innerHTML = `<div class="donut-wrap">${svg}${legend}</div>`;
  }
