...
function sortList(list) {
  ...
  switch (state.sort) {
    ...
    default: // relevance: rating desc as a sensible default ordering
...
function renderEmpty() {
  ...
  if (best) {
    detail.textContent = `Your current filters are too narrow. Relaxing the ${FACET_LABEL[best.facet]} filter would show ${best.n} ${best.n === 1 ? 'product' : 'products'}.`;
    relaxBtn.hidden = false;
    relaxBtn.textContent = `Relax ${FACET_LABEL[best.facet]} filter`;
