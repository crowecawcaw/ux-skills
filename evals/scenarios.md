...
Every app ships a `brief.md` (same shape as `evals/832events-brief.md`) and
happy-path `scenarios/*.json` for the screenshot helper. Surface types span the
three review lenses plus surfaces deliberately outside them.
...
These three are also the subjects of the review→address loop eval. The rest are
documented here as the backlog.
